...
show_usage() {
    cat << EOF
clawlog - StableClaw Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View StableClaw logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    StableClaw logs flow through the macOS unified log (subsystem: ai.openclaw).
