...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "stableclaw" | "openclaw" | "clawdbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["stableclaw", "openclaw", "clawdbot"] as const;
...
function isOpenClawGatewaySystemdService(name: string, contents: string): boolean {
  ...
  if (!name.startsWith("stableclaw-gateway") && !name.startsWith("openclaw-gateway")) {
    return false;
  }
...
function isOpenClawGatewayTaskName(name: string): boolean {
  ...
  return normalized === defaultName || normalized.startsWith("stableclaw gateway") || normalized.startsWith("openclaw gateway");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    if ((marker === "stableclaw" || marker === "openclaw") && isOpenClawGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
...
async function scanSystemdDir(params: {
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if ((marker === "stableclaw" || marker === "openclaw") && isOpenClawGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
