...
async function activateSystemdService(params: { env: GatewayServiceEnv }) {
  const serviceName = resolveGatewaySystemdServiceName(params.env.STABLECLAW_PROFILE || params.env.OPENCLAW_PROFILE);
...
export async function uninstallSystemdService({
  ...
  await assertSystemdAvailable(env);
  const serviceName = resolveGatewaySystemdServiceName(env.STABLECLAW_PROFILE || env.OPENCLAW_PROFILE);
  const unitName = `${serviceName}.service`;
