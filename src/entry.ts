...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "stableclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "stableclaw.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureCliRespawnReady(): boolean {
    ...
    child.once("error", (error) => {
      console.error(
        "[stableclaw] Failed to respawn CLI:",
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      ...
      .catch((error) => {
        console.error(
          "[stableclaw] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
...
export function tryHandleRootHelpFastPath(
  ...
  const handleError =
    ...
    ((error: unknown) => {
      console.error(
        "[stableclaw] Failed to display help:",
        error instanceof Error ? (error.stack ?? error.message) : error,
      );
      process.exitCode = 1;
    });
...
function runMainOrRootHelp(argv: string[]): void {
  ...
  import("./cli/run-main.js")
    ...
    .catch((error) => {
      console.error(
        "[stableclaw] Failed to start CLI:",
        error instanceof Error ? (error.stack ?? error.message) : error,
      );
      process.exitCode = 1;
    });
