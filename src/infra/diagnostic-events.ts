...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __stableclawDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__stableclawDiagnosticEventsState) {
    globalStore.__stableclawDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__stableclawDiagnosticEventsState;
}
