...
describe("packNpmSpecToArchive", () => {
  ...
  it("returns friendly error for 404 (package not on npm)", async () => {
    ...
    mockPackCommandResult({
      stdout: "",
      stderr: "npm error code E404\nnpm error 404  '@stableclaw/whatsapp@*' is not in this registry.",
      code: 1,
    });

    const result = await runPack("@stableclaw/whatsapp", cwd);
    expectPackError(result, [
      "Package not found on npm",
      "@stableclaw/whatsapp",
      "docs.stableclaw.ai/tools/plugin",
    ]);
  });
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    ...
    mockPackCommandResult({
      stdout:
        ...
        JSON.stringify([
          {
            id: "@stableclaw/plugin-demo@2.0.0",
            filename: "openclaw-plugin-demo-2.0.0.tgz",
          },
        ]),
    });

    const result = await runPack("@stableclaw/plugin-demo@2.0.0", cwd);
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "openclaw-plugin-demo-2.0.0.tgz"),
      metadata: {
        resolvedSpec: "@stableclaw/plugin-demo@2.0.0",
      },
    });
  });
