...
// Legacy state dirs: .clawdbot (original) and .openclaw (upstream).
...
export function resolveStateDir(
  ...
  if (env.STABLECLAW_TEST_FAST === "1" || env.OPENCLAW_TEST_FAST === "1") {
    return newDir;
  }
...
export function resolveGatewayPort(
  ...
  const envRaw = env.STABLECLAW_GATEWAY_PORT?.trim() || env.OPENCLAW_GATEWAY_PORT?.trim();
