...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.stableclawCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.stableclawCanvasA2UIAction &&
        typeof window.stableclawCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.stableclawSendUserAction === "function";
  ...
  window.addEventListener("stableclaw:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android StableClaw node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.stableclawSendUserAction === "function"
        ? window.stableclawSendUserAction
        : undefined;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>StableClaw Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
