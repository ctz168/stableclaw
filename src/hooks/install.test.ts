...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@stableclaw/test-hooks@0.0.1",
              name: "@stableclaw/test-hooks",
              version: "0.0.1",
              filename: packedName,
    ...
    const result = await installHooksFromNpmSpec({
      spec: "@stableclaw/test-hooks@0.0.1",
      hooksDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@stableclaw/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@stableclaw/test-hooks@0.0.1",
    });
  ...
  it("rejects invalid npm spec shapes", async () => {
    ...
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@stableclaw/test-hooks@0.0.2-beta.1",
      name: "@stableclaw/test-hooks",
    ...
    const result = await installHooksFromNpmSpec({
      spec: "@stableclaw/test-hooks",
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("prerelease version 0.0.2-beta.1");
      expect(result.error).toContain('"@stableclaw/test-hooks@beta"');
    }
  });
});
