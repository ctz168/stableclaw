...
If you believe you've found a security issue in StableClaw, please report it privately.

## Reporting

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [stableclaw/stableclaw](https://github.com/stableclaw/stableclaw)
- **macOS desktop app** — [stableclaw/stableclaw](https://github.com/stableclaw/stableclaw) (apps/macos)
- **iOS app** — [stableclaw/stableclaw](https://github.com/stableclaw/stableclaw) (apps/ios)
- **Android app** — [stableclaw/stableclaw](https://github.com/stableclaw/stableclaw) (apps/android)
- **ClawHub** — [stableclaw/clawhub](https://github.com/stableclaw/clawhub)
- **Trust and threat model** — [stableclaw/trust](https://github.com/stableclaw/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@stableclaw.ai](mailto:security@stableclaw.ai)** and we'll route it.

For full reporting instructions see our [Trust page](https://trust.stableclaw.ai).
...
- Tested version details (StableClaw version and/or commit SHA).
...
- Demonstrated impact tied to StableClaw's documented trust boundaries.
- For exposed-secret reports: proof the credential is StableClaw-owned (or grants access to StableClaw-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at StableClaw. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

StableClaw is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
StableClaw does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- Concretely, on the OpenAI-compatible HTTP surface:
  ...
  - narrower `x-stableclaw-scopes` headers are ignored for that shared-secret path
...
- StableClaw can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need StableClaw, use one VPS (or host/OS user boundary) per user.
...
- This is expected in StableClaw's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of StableClaw's trusted computing base for a gateway.
...
- Using StableClaw in ways that the docs recommend not to
...
- Reports that require write access to trusted local state (`~/.stableclaw`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not StableClaw-owned and not granting access to StableClaw-operated infrastructure/services) without demonstrated StableClaw impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.

## Deployment Assumptions

StableClaw security guidance assumes:

- The host where StableClaw runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.stableclaw` state/config (including `stableclaw.json`) is effectively a trusted operator.
...
StableClaw's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
StableClaw separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when StableClaw can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the StableClaw process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

StableClaw uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/stableclaw` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/stableclaw` (or `stableclaw-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the StableClaw-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use StableClaw temp helpers (`resolvePreferredStableClawTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-stableclaw-dir.ts`
...
For threat model + hardening guidance (including `stableclaw security audit --deep` and `--fix`), see:

- `https://docs.stableclaw.ai/gateway/security`
...
StableClaw's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `stableclaw gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - StableClaw keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `stableclaw security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__stableclaw__/canvas/`, `/__stableclaw__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__stableclaw__/canvas/`, `/__stableclaw__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

StableClaw requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running StableClaw in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v stableclaw-data:/app/data \
  stableclaw/stableclaw:latest
```
