...
import { o as resolveCompatibilityHostVersion } from "./version-BNzJ37Re.js";
import { i as openBoundaryFileSync, n as matchBoundaryFileOpenFailure } from "./boundary-file-read-DB4ibbUg.js";
import { h as resolveBundledPluginsDir } from "./chat-meta-wVbKo1sw.js";
import { a as resolvePackageExtensionEntries, i as loadPluginManifest, r as getPackageManifestMetadata, t as DEFAULT_PLUGIN_ENTRY_CANDIDATES } from "./manifest-f_gihArl.js";
import { a as normalizePluginsConfig } from "./config-state-DfIk9ME6.js";
import { a as safeStatSync, i as safeRealpathSync, l as detectBundleManifestFormat, n as formatPosixMode, r as isPathInside, t as checkMinHostVersion, u as loadBundleManifest } from "./min-host-version-CIhgRq0c.js";
import fsSync from "node:fs";
import path from "node:path";
//#region src/plugins/persistent-cache.ts
/**
...
*/
function resolveCacheFilePath(env) {
...
*/
function calculateManifestChecksum(manifestPath) {
	try {
...
*/
function loadPersistentCache(env) {
	try {
...
*/
function savePersistentCache(result, roots, env) {
	try {
		const cachePath = resolveCacheFilePath(env);
		const cachedCandidates = result.candidates.map((candidate) => ({
			...
			manifestChecksum: calculateManifestChecksum(path.join(candidate.rootDir, "openclaw.plugin.json")),
...
*/
function isCacheValid(cache, roots) {
	if (cache.roots.stock !== roots.stock || cache.roots.global !== roots.global || cache.roots.workspace !== roots.workspace) return false;
	for (const cached of cache.candidates) if (calculateManifestChecksum(path.join(cached.rootDir, "openclaw.plugin.json")) !== cached.manifestChecksum) return false;
	return true;
}
/**
* Convert cached candidates back to discovery result.
*/
function cachedCandidatesToResult(cache) {
	return {
		candidates: cache.candidates.map((cached) => ({
...
*/
function clearPersistentCache(env) {
	try {
		const cachePath = resolveCacheFilePath(env);
		if (fsSync.existsSync(cachePath)) fsSync.unlinkSync(cachePath);
	} catch (error) {}
}
//#endregion
...
const DEFAULT_DISCOVERY_CACHE_MS = 1e4;
...
function discoverInDirectory(params) {
	...
	if (params.failFast && params.diagnostics.some((d) => d.level === "error")) return;
	let entries = [];
	try {
...
function discoverOpenClawPlugins(params) {
	...
	if (cacheEnabled) {
		...
		if (persistentCache && isCacheValid(persistentCache, roots)) return cachedCandidatesToResult(persistentCache);
	}
	...
	for (const extraPath of extra) {
		...
		if (params.failFast && diagnostics.some((d) => d.level === "error")) return {
			candidates,
			diagnostics
		};
	}
