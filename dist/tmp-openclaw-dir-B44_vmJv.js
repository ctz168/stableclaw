...
const POSIX_OPENCLAW_TMP_DIR = "/tmp/stableclaw";
...
function resolvePreferredOpenClawTmpDir(options = {}) {
	...
	const fallback = () => {
		const base = tmpdir$1();
		const suffix = uid === void 0 ? "stableclaw" : `stableclaw-${uid}`;
	...
	const tryRepairWritableBits = (candidatePath) => {
		try {
			...
			warn(`[stableclaw] tightened permissions on temp dir: ${candidatePath}`);
	...
	const ensureTrustedFallbackDir = () => {
		...
		if (state === "invalid") {
			...
			throw new Error(`Unsafe fallback StableClaw temp dir: ${fallbackPath}`);
		}
		try {
			...
			throw new Error(`Unable to create fallback StableClaw temp dir: ${fallbackPath}`);
		}
		if (resolveDirState(fallbackPath) !== "available" && !tryRepairWritableBits(fallbackPath)) throw new Error(`Unsafe fallback StableClaw temp dir: ${fallbackPath}`);
		return fallbackPath;
	};
	const existingPreferredState = resolveDirState(POSIX_OPENCLAW_TMP_DIR);
	if (existingPreferredState === "available") return POSIX_OPENCLAW_TMP_DIR;
	if (existingPreferredState === "invalid") {
		if (tryRepairWritableBits("/tmp/stableclaw")) return POSIX_OPENCLAW_TMP_DIR;
	...
	try {
		...
		if (resolveDirState("/tmp/stableclaw") !== "available" && !tryRepairWritableBits("/tmp/stableclaw")) return ensureTrustedFallbackDir();
