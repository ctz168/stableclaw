import { d as resolveGatewayServiceDescription, l as resolveGatewayLaunchAgentLabel, m as resolveLegacyGatewayLaunchAgentLabels, t as GATEWAY_LAUNCH_AGENT_LABEL } from "./constants-Bq55qi82.js";
import { i as parseStrictPositiveInteger, n as parseStrictInteger } from "./parse-finite-number-CzdLC_gQ.js";
import { t as cleanStaleGatewayProcessesSync } from "./restart-stale-pids-dsB3eE6_.js";
import { a as toPosixPath, i as formatLine, n as resolveGatewayStateDir, o as writeFormattedLines, r as resolveHomeDir, s as execFileUtf8, t as parseKeyValueOutput } from "./runtime-parse-vvG9x7Ps.js";
...
function resolveLaunchAgentLabel(args) {
	const envLabel = (args?.env?.STABLECLAW_LAUNCHD_LABEL || args?.env?.OPENCLAW_LAUNCHD_LABEL)?.trim();
	if (envLabel) return envLabel;
	return resolveGatewayLaunchAgentLabel(args?.env?.STABLECLAW_PROFILE || args?.env?.OPENCLAW_PROFILE);
}
...
function resolveGatewayLogPaths(env) {
	...
	const prefix = (env.STABLECLAW_LOG_PREFIX || env.OPENCLAW_LOG_PREFIX)?.trim() || "gateway";
	return {
		logDir,
...
async function resolveLaunchAgentGatewayPort(env) {
	...
	return parseStrictPositiveInteger((env.STABLECLAW_GATEWAY_PORT || env.OPENCLAW_GATEWAY_PORT) ?? "") ?? null;
}
...
function throwBootstrapGuiSessionError(params) {
	throw new Error([
		...
		"Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.stableclaw.ai/gateway"
	].join("\n"));
}
...
async function writeLaunchAgentPlist({ env, programArguments, workingDirectory, environment, description }) {
	...
	const domain = resolveGuiDomain();
	const label = resolveLaunchAgentLabel({ env });
	for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.STABLECLAW_PROFILE || env.OPENCLAW_PROFILE)) {
...
async function activateLaunchAgent(params) {
	...
	await bootstrapLaunchAgentOrThrow({
		...
		actionHint: "stableclaw gateway install --force"
	});
}
...
async function restartLaunchAgent({ stdout, env }) {
	...
	await bootstrapLaunchAgentOrThrow({
		...
		serviceTarget,
		plistPath,
		actionHint: "stableclaw gateway restart"
	});
