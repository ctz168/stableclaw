...
import { t as isTailnetIPv4 } from "./tailnet-CZMxMuGK.js";
...
const GATEWAY_SERVICE_TYPE = "_stableclaw-gw._tcp";
...
function parseDnsSdBrowse(stdout) {
	...
	for (const raw of stdout.split("\n")) {
		...
		const match = line.match(/_stableclaw-gw\._tcp\.?\s+(.+)$/);
...
async function discoverWideAreaViaTailnetDns(domain, timeoutMs, run) {
	...
	for (const ptr of ptrs) {
		...
		if (!ptrName) continue;
		const instanceName = ptrName.replace(/\.?_stableclaw-gw\._tcp\..*$/, "");
		const srv = await run([
			"dig",
			"+short",
			"+time=1",
