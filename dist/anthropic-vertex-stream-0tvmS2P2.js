...
import { o as normalizeModelCompat } from "./provider-model-shared-GO1WT2fY.js";
...
import { f as isSecretRefHeaderValueMarker } from "./model-auth-markers-D2z_AIuC.js";
import { i as discoverModels, r as discoverAuthStorage } from "./pi-model-discovery-MzecpVik.js";
import { i as resolveProviderRequestConfig } from "./provider-request-config-CIrbwAnR.js";
import { n as shouldSuppressBuiltInModel, t as buildSuppressedBuiltInModelError } from "./model-suppression-Czi-yw_Z.js";
import { c as resolveAnthropicVertexProjectId, o as resolveAnthropicVertexClientRegion } from "./anthropic-vertex-s0m7nQhq.js";
...
function findInlineModelMatch(params) {
	...
	const normalized = inlineModels.find((entry) => normalizeProviderId(entry.provider) === normalizedProvider && entry.id === params.modelId);
	if (normalized) return normalized;
	const byName = inlineModels.find((entry) => normalizeProviderId(entry.provider) === normalizedProvider && entry.name === params.modelId);
...
function resolveConfiguredFallbackModel(params) {
	...
	const providerConfig = resolveConfiguredProviderConfig(cfg, provider);
	const configuredModel = providerConfig?.models?.find((candidate) => candidate.id === modelId) ?? providerConfig?.models?.find((candidate) => candidate.name === modelId);
	const providerHeaders = sanitizeModelHeaders(providerConfig?.headers, { stripSecretRefMarkers: true });
