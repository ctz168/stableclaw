...
import { d as resolveGatewayServiceDescription, f as resolveGatewaySystemdServiceName, i as LEGACY_GATEWAY_SYSTEMD_SERVICE_NAMES } from "./constants-Bq55qi82.js";
import { i as parseStrictPositiveInteger, n as parseStrictInteger } from "./parse-finite-number-CzdLC_gQ.js";
import { a as toPosixPath, i as formatLine, o as writeFormattedLines, r as resolveHomeDir, s as execFileUtf8, t as parseKeyValueOutput } from "./runtime-parse-vvG9x7Ps.js";
import { t as splitArgsPreservingQuotes } from "./arg-split-get0R_Yr.js";
...
function resolveSystemdServiceName(env) {
	const override = (env.STABLECLAW_SYSTEMD_UNIT || env.OPENCLAW_SYSTEMD_UNIT)?.trim();
	if (override) return override.endsWith(".service") ? override.slice(0, -8) : override;
	return resolveGatewaySystemdServiceName(env.STABLECLAW_PROFILE || env.OPENCLAW_PROFILE);
}
...
async function activateSystemdService(params) {
	const unitName = `${resolveGatewaySystemdServiceName(params.env.STABLECLAW_PROFILE || params.env.OPENCLAW_PROFILE)}.service`;
...
async function uninstallSystemdService({ env, stdout }) {
	...
	await execSystemctlUser(env, [
		"disable",
		"--now",
		`${resolveGatewaySystemdServiceName(env.STABLECLAW_PROFILE || env.OPENCLAW_PROFILE)}.service`
	]);
	const unitPath = resolveSystemdUnitPath(env);
	try {
