import { b as shortenHomeInString, g as resolveHomeDir, m as pathExists, v as resolveUserPath, x as shortenHomePath } from "./utils-CN_F_3Qg.js";
...
import { t as formatSessionArchiveTimestamp } from "./artifacts-DXKlQp1z.js";
...
async function createBackupArchive(opts = {}) {
	...
	if (plan.included.length === 0) throw new Error(onlyConfig ? "No StableClaw config file was found to back up." : "No local StableClaw state was found to back up.");
	...
	const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "stableclaw-backup-"));
