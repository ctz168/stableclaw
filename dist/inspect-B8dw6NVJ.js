import { f as resolveGatewaySystemdServiceName, l as resolveGatewayLaunchAgentLabel, p as resolveGatewayWindowsTaskName, r as GATEWAY_SERVICE_MARKER } from "./constants-Bq55qi82.js";
import { r as resolveHomeDir } from "./runtime-parse-vvG9x7Ps.js";
import { a as execSchtasks } from "./service-2O78fADl.js";
...
function hasGatewayServiceMarker(content) {
	const lower = content.toLowerCase();
	const markerKeys = ["stableclaw_service_marker", "openclaw_service_marker"];
	const kindKeys = ["stableclaw_service_kind", "openclaw_service_kind"];
...
function isOpenClawGatewayLaunchdService(label, contents) {
	...
	return label.startsWith("ai.stableclaw.") || label.startsWith("ai.openclaw.");
}
function isOpenClawGatewaySystemdService(name, contents) {
	if (hasGatewayServiceMarker(contents)) return true;
	if (!name.startsWith("stableclaw-gateway") && !name.startsWith("openclaw-gateway")) return false;
	return contents.toLowerCase().includes("gateway");
}
function isOpenClawGatewayTaskName(name) {
	const normalized = name.trim().toLowerCase();
	if (!normalized) return false;
	return normalized === resolveGatewayWindowsTaskName().toLowerCase() || normalized.startsWith("stableclaw gateway") || normalized.startsWith("openclaw gateway");
}
...
async function scanLaunchdDir(params) {
	...
	for (const { name: labelFromName, fullPath, contents } of candidates) {
		...
		if ((marker === "stableclaw" || marker === "openclaw") && isOpenClawGatewayLaunchdService(label, contents)) continue;
		results.push({
			platform: "darwin",
			label,
			detail: `plist: ${fullPath}`,
			scope: params.scope,
			marker,
			legacy: marker !== "stableclaw" || isLegacyLabel(label)
		});
	}
	return results;
}
async function scanSystemdDir(params) {
	...
	for (const { entry, name, fullPath, contents } of candidates) {
		...
		if (!marker) continue;
		if ((marker === "stableclaw" || marker === "openclaw") && isOpenClawGatewaySystemdService(name, contents)) continue;
		results.push({
			platform: "linux",
			label: entry,
