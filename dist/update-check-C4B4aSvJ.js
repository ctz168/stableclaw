...
import { i as parseComparableSemver, t as compareComparableSemver } from "./semver-compare-DkYCMcwG.js";
import { r as fetchWithTimeout } from "./fetch-timeout-BKMGkHu9.js";
import { r as channelToNpmTag } from "./update-channels-Cqiboku6.js";
...
async function fetchNpmPackageTargetStatus(params) {
	...
	try {
		const res = await fetchWithTimeout(`https://registry.npmjs.org/stableclaw/${encodeURIComponent(target)}`, {}, Math.max(250, timeoutMs));
		if (!res.ok) return {
			target,
			version: null,
			nodeEngine: null,
