...
import { f as readConfigFileSnapshot, x as writeConfigFile } from "./io-8kNEV_ou.js";
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-CShuKltl.js");
...
async function requireRiskAcknowledgement(params) {
	...
	await params.prompter.note([
		...
		"",
		"StableClaw is a hobby project and still in beta. Expect sharp edges.",
		"By default, StableClaw is a personal agent: one trusted operator boundary.",
		"This bot can read files and run actions if tools are enabled.",
		"A bad prompt can trick it into doing unsafe things.",
		"",
		"StableClaw is not a hostile multi-tenant boundary by default.",
		"If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
		"",
		"If you’re not comfortable with security hardening and access control, don't run StableClaw.",
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-D09CeEb1.js");
	onboardHelpers.printWizardHeader(runtime);
	await prompter.intro("StableClaw setup");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-CquTn9JV.js");
		const { logConfigUpdated } = await import("./logging-DZ4VEhS4.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-DNpCRGsb.js");
	let nextConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
	const { ensureAuthProfileStore } = await import("./agents/auth-profiles.runtime.js");
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-CXmiOLyw.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-90cPEZ2d.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-C3zkpuBp.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-pMD4gjOu.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-CadQtFz8.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-DcjzAZWt.js");
		const { setupChannels } = await import("./onboard-channels-B92ZJdl1.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-C3bJkqG7.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-DvFtFv4Y.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	const { setupInternalHooks } = await import("./onboard-hooks-C7UHhFcz.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-B7Q9Q2h3.js");
