...
import { N as isRootHelpInvocation, P as isRootVersionInvocation, j as hasHelpOrVersion } from "./logger-MWEZrHUL.js";
import { a as parseCliContainerArgs, n as applyCliProfileEnv, o as resolveCliContainerTarget, r as parseCliProfileArgs, t as normalizeWindowsArgv } from "./windows-argv-DDAf3waz.js";
...
import { t as ensureOpenClawExecMarkerOnProcess } from "./openclaw-exec-env-DnTrPIdx.js";
import { t as installProcessWarningFilter } from "./warning-filter-GX8LpGKz.js";
...
const ENTRY_WRAPPER_PAIRS = [{
	wrapperBasename: "stableclaw.mjs",
	entryBasename: "entry.js"
}, {
	wrapperBasename: "stableclaw.js",
	entryBasename: "entry.js"
}];
...
if (!isMainModule({
	...
	const { installGaxiosFetchCompat } = await import("./gaxios-fetch-compat-BdTZ2WeU.js");
	await installGaxiosFetchCompat();
	process$1.title = "stableclaw";
	...
	function ensureCliRespawnReady() {
		...
		child.once("error", (error) => {
			console.error("[stableclaw] Failed to respawn CLI:", error instanceof Error ? error.stack ?? error.message : error);
			process$1.exit(1);
		});
		return true;
	}
	function tryHandleRootVersionFastPath(argv) {
		...
		Promise.all([import("./version-Dw0eJ-1F.js"), import("./git-commit-BS28SZVT.js")]).then(([{ VERSION }, { resolveCommitHash }]) => {
			const commit = resolveCommitHash({ moduleUrl: import.meta.url });
			console.log(commit ? `StableClaw ${VERSION} (${commit})` : `StableClaw ${VERSION}`);
			process$1.exit(0);
		}).catch((error) => {
			console.error("[stableclaw] Failed to resolve version:", error instanceof Error ? error.stack ?? error.message : error);
			process$1.exitCode = 1;
		});
		return true;
	}
	...
	if (!ensureCliRespawnReady()) {
		...
		if (!parsedContainer.ok) {
			console.error(`[stableclaw] ${parsedContainer.error}`);
			process$1.exit(2);
		}
		const parsed = parseCliProfileArgs(parsedContainer.argv);
		if (!parsed.ok) {
			console.error(`[stableclaw] ${parsed.error}`);
			process$1.exit(2);
		}
		if (resolveCliContainerTarget(process$1.argv) && parsed.profile) {
			console.error("[stableclaw] --container cannot be combined with --profile/--dev");
			process$1.exit(2);
		}
		if (parsed.profile) {
...
function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	const handleError = deps.onError ?? ((error) => {
		console.error("[stableclaw] Failed to display help:", error instanceof Error ? error.stack ?? error.message : error);
		process$1.exitCode = 1;
	});
	...
	import("./root-help-C56v-rtM.js").then(({ outputRootHelp }) => {
...
function runMainOrRootHelp(argv) {
	...
	import("./run-main-CApJuAuY.js").then(({ runCli }) => runCli(argv)).catch((error) => {
		console.error("[stableclaw] Failed to start CLI:", error instanceof Error ? error.stack ?? error.message : error);
		process$1.exitCode = 1;
	});
}
//#endregion
