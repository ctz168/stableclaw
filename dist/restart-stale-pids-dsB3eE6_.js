...
import { n as resolveLsofCommandSync } from "./ports-lsof-Cw_6T7ru.js";
...
* returning control to the caller (typically `triggerStableClawRestart` →
...
function parsePidsFromLsofOutput(stdout) {
	...
	for (const line of stdout.split(/\r?\n/).filter(Boolean)) if (line.startsWith("p")) {
		if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("stableclaw")) pids.push(currentPid);
