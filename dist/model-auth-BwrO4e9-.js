...
import { o as getRuntimeConfigSnapshot } from "./io-8kNEV_ou.js";
import { t as getShellEnvAppliedKeys } from "./shell-env-BTztt4a1.js";
...
import { i as resolveOwningPluginIdsForProvider } from "./providers-DUy2TaM-.js";
import { M as resolveProviderSyntheticAuthWithPlugin, o as buildProviderMissingAuthMessageWithPlugin } from "./provider-runtime-YHMKDzAl.js";
import { i as NON_ENV_SECRETREF_MARKER, l as isKnownEnvApiKeyMarker, t as CUSTOM_LOCAL_AUTH_MARKER, u as isNonSecretApiKeyMarker } from "./model-auth-markers-D2z_AIuC.js";
...
function shouldDeferSyntheticOllamaProfileAuth(params) {
	return normalizeProviderId(params.provider) === "ollama" && params.resolvedApiKey?.trim() === "ollama-local";
}
async function resolveApiKeyForProvider(params) {
	...
	if (profileId) {
		...
		if (!params.lockedProfile && shouldDeferSyntheticOllamaProfileAuth({
			provider,
	...
	for (const candidate of order) try {
		...
		if (resolved) {
			...
			if (shouldDeferSyntheticOllamaProfileAuth({
				provider,
				resolvedApiKey: resolved.apiKey
			})) {
