...
import { n as extractArchive, o as resolvePackedRootDir, r as fileExists } from "./archive-BaCqqQP1.js";
import { i as withTempDir } from "./install-source-utils-CxNODq7h.js";
import { t as assertCanonicalPathWithinBase } from "./install-safe-path-Dk_ccRad.js";
...
const STAGED_NPM_PROJECT_CONFIG_PREFIX = ".stableclaw-install-hidden-npmrc-";
...
async function installPackageDir(params) {
	...
	try {
		...
		stageDir = await fs.mkdtemp(path.join(installBaseRealPath, ".stableclaw-install-stage-"));
	...
	if (params.mode === "update" && await fileExists(canonicalTargetDir)) {
		const backupRoot = path.join(installBaseRealPath, ".stableclaw-install-backups");
