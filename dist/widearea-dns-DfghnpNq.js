import { c as ensureDir, t as CONFIG_DIR } from "./utils-CN_F_3Qg.js";
...
function extractContentHash(zoneText) {
	return zoneText.match(/^\s*;\s*(?:stableclaw)-content-hash:\s*(\S+)\s*$/m)?.[1] ?? null;
}
...
function renderZone(opts) {
	const hostname = os.hostname().split(".")[0] ?? "stableclaw";
	const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "stableclaw");
	const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "stableclaw-gw");
	...
	records.push(`_stableclaw-gw._tcp IN PTR ${instanceLabel}._stableclaw-gw._tcp`);
	records.push(`${instanceLabel}._stableclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
	records.push(`${instanceLabel}._stableclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
	const contentBody = `${records.join("\n")}\n`;
	return `; stableclaw-content-hash: ${computeContentHash(`${records.map((line) => line === soaLine ? `@ IN SOA ns1 hostmaster SERIAL 7200 3600 1209600 60` : line).join("\n")}\n`)}\n${contentBody}`;
}
