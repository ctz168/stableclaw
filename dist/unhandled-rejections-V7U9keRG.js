import { i as formatUncaughtError, n as extractErrorCode, s as readErrorName, t as collectErrorGraphCandidates } from "./errors-Bs2h5H8p.js";
...
function hasSqliteSignal(err) {
	...
	if (readErrorName(err).toLowerCase().includes("sqlite")) return true;
	if (("message" in err && typeof err.message === "string" ? err.message : "").toLowerCase().includes("sqlite")) return true;
	return false;
}
...
function isUnhandledRejectionHandled(reason) {
	for (const handler of handlers) try {
		...
		console.error("[stableclaw] Unhandled rejection handler failed:", err instanceof Error ? err.stack ?? err.message : err);
	}
	return false;
}
function installUnhandledRejectionHandler() {
	process.on("unhandledRejection", (reason, _promise) => {
		...
		if (isAbortError(reason)) {
			console.warn("[stableclaw] Suppressed AbortError:", formatUncaughtError(reason));
			return;
		}
		if (isFatalError(reason)) {
			console.error("[stableclaw] FATAL unhandled rejection:", formatUncaughtError(reason));
			process.exit(1);
			return;
		}
		if (isConfigError(reason)) {
			console.error("[stableclaw] CONFIGURATION ERROR - requires fix:", formatUncaughtError(reason));
			process.exit(1);
			return;
		}
		if (isTransientUnhandledRejectionError(reason)) {
			console.warn("[stableclaw] Non-fatal unhandled rejection (continuing):", formatUncaughtError(reason));
			return;
		}
		console.error("[stableclaw] Unhandled promise rejection:", formatUncaughtError(reason));
		process.exit(1);
	});
}
//#endregion
