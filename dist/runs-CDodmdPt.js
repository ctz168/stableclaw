...
import { a as getRuntimeConfig } from "./io-8kNEV_ou.js";
...
import { l as logSessionStateChange, s as logMessageQueued, t as diag } from "./diagnostic-DLLxcEb3.js";
...
function triggerStableClawRestart() {
...
function consumeEmbeddedRunModelSwitch(sessionId) {
	...
	if (request) EMBEDDED_RUN_MODEL_SWITCH_REQUESTS.delete(normalizedSessionId);
	return request;
}
/**
...
async function waitForActiveEmbeddedRuns(timeoutMs = 15e3, opts) {
	...
	const startedAt = Date.now();
	while (true) {
		if (ACTIVE_EMBEDDED_RUNS.size === 0) return { drained: true };
		if (Date.now() - startedAt >= maxWaitMs) {
			diag.warn(`wait for active embedded runs timed out: activeRuns=${ACTIVE_EMBEDDED_RUNS.size} timeoutMs=${maxWaitMs}`);
...
//#endregion
export { markGatewaySigusr1RestartHandled as _, getActiveEmbeddedRunSnapshot as a, setPreRestartDeferralCheck as b, queueEmbeddedPiMessage as c, waitForActiveEmbeddedRuns as d, waitForEmbeddedPiRunEnd as f, isGatewaySigusr1RestartExternallyAllowed as g, emitGatewayRestart as h, getActiveEmbeddedRunCount as i, setActiveEmbeddedRun as l, deferGatewayRestartUntilIdle as m, clearActiveEmbeddedRun as n, isEmbeddedPiRunActive as o, consumeGatewaySigusr1RestartAuthorization as p, consumeEmbeddedRunModelSwitch as r, isEmbeddedPiRunStreaming as s, abortEmbeddedPiRun as t, updateActiveEmbeddedRunSnapshot as u, scheduleGatewaySigusr1Restart as v, triggerStableClawRestart as x, setGatewaySigusr1RestartPolicy as y };
