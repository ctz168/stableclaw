...
import { n as resolveBrewPathDirs } from "./brew-B9JEV5Jj.js";
...
function candidateBinDirs(opts) {
	...
	try {
		...
		if (isExecutable(path.join(execDir, "stableclaw"))) prepend.push(execDir);
	} catch {}
	if (opts.allowProjectLocalBin === true || isTruthyEnvValue(process.env.OPENCLAW_ALLOW_PROJECT_LOCAL_BIN)) {
		const localBinDir = path.join(cwd, "node_modules", ".bin");
		if (isExecutable(path.join(localBinDir, "stableclaw"))) append.push(localBinDir);
	}
