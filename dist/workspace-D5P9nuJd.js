...
import { C as isCronSessionKey, w as isSubagentSessionKey } from "./session-key-Do__tq1E.js";
...
function resolveDefaultAgentWorkspaceDir(env = process.env, homedir = os.homedir) {
	...
	const profile = (env.STABLECLAW_PROFILE || env.OPENCLAW_PROFILE)?.trim();
	if (profile && profile.toLowerCase() !== "default") return path.join(home, ".stableclaw", `workspace-${profile}`);
	return path.join(home, ".stableclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".stableclaw";
