...
import { d as resolveGatewayServiceDescription, p as resolveGatewayWindowsTaskName } from "./constants-Bq55qi82.js";
import { t as getWindowsInstallRoots } from "./windows-install-roots-XI3CvO9O.js";
import { n as findGatewayPidsOnPortSync } from "./restart-stale-pids-dsB3eE6_.js";
import { i as formatLine, n as resolveGatewayStateDir, o as writeFormattedLines, t as parseKeyValueOutput } from "./runtime-parse-vvG9x7Ps.js";
import { a as readLaunchAgentProgramArguments, d as stageLaunchAgent, f as stopLaunchAgent, o as readLaunchAgentRuntime, p as uninstallLaunchAgent, r as isLaunchAgentLoaded, t as installLaunchAgent, u as restartLaunchAgent } from "./launchd-cp98MUhz.js";
import { a as renderCmdSetAssignment, i as parseCmdSetAssignment, n as quoteCmdScriptArg, r as assertNoCmdLineBreak, t as parseCmdScriptCommandLine } from "./cmd-argv-BbyQhzAC.js";
import { a as inspectPortUsage } from "./ports-ZA1n2It7.js";
import { t as killProcessTree } from "./kill-tree-CUPpLbeq.js";
import { a as readSystemdServiceExecStart, c as restartSystemdService, f as uninstallSystemdService, l as stageSystemdService, o as readSystemdServiceRuntime, r as isSystemdServiceEnabled, t as installSystemdService, u as stopSystemdService } from "./systemd-B2HBHMpF.js";
...
function isGatewayArgv(args, opts) {
	...
	return exe.endsWith("/stableclaw") || exe === "stableclaw" || opts?.allowGatewayBinary === true && exe.endsWith("/stableclaw-gateway");
}
//#endregion
...
function resolveTaskName(env) {
	const override = (env.STABLECLAW_WINDOWS_TASK_NAME || env.OPENCLAW_WINDOWS_TASK_NAME)?.trim();
	if (override) return override;
	return resolveGatewayWindowsTaskName(env.STABLECLAW_PROFILE || env.OPENCLAW_PROFILE);
}
...
function resolveTaskScriptPath(env) {
	const override = (env.STABLECLAW_TASK_SCRIPT || env.OPENCLAW_TASK_SCRIPT)?.trim();
	if (override) return override;
	const scriptName = (env.STABLECLAW_TASK_SCRIPT_NAME || env.OPENCLAW_TASK_SCRIPT_NAME)?.trim() || "gateway.cmd";
...
function resolveConfiguredGatewayPort(env) {
	const raw = (env.STABLECLAW_GATEWAY_PORT || env.OPENCLAW_GATEWAY_PORT)?.trim();
	if (!raw) return null;
...
async function resolveScheduledTaskPort(env) {
	...
	return parsePortFromProgramArguments(command?.programArguments) ?? parsePositivePort(command?.environment?.STABLECLAW_GATEWAY_PORT || command?.environment?.OPENCLAW_GATEWAY_PORT) ?? resolveConfiguredGatewayPort(env);
}
...
async function activateScheduledTask(params) {
	const taskDescription = params.description ?? "StableClaw Gateway";
...
//#endregion
export { execSchtasks as a, signalVerifiedGatewayPidSync as c, parseWindowsCmdline as d, startGatewayService as i, isGatewayArgv as l, readGatewayServiceState as n, findVerifiedGatewayListenerPidsOnPortSync as o, resolveGatewayService as r, formatGatewayPidList as s, describeGatewayServiceRestart as t, parseProcCmdline as u };
