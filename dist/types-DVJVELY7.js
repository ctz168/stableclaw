...
import { o as detachPluginConversationBinding, p as requestPluginConversationBinding, s as getCurrentPluginConversationBinding } from "./conversation-binding-C2nuZ3Eg.js";
...
function validatePluginCommandDefinition(command) {
	...
	if ("telegramNativeProgressMessage" in command) {
		if (typeof command.telegramNativeProgressMessage !== "string") return "telegramNativeProgressMessage must be a string";
		if (!command.telegramNativeProgressMessage.trim()) return "telegramNativeProgressMessage cannot be empty";
	}
	return null;
}
