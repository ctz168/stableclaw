...
import { A as hasFlag, D as getPositiveIntFlagValue, E as getFlagValue, L as isValueToken, N as isRootHelpInvocation, O as getPrimaryCommand, T as getCommandPositionalsWithRootOptions, g as loggingState, j as hasHelpOrVersion, k as getVerboseFlag, w as getCommandPathWithRootOptions } from "./logger-MWEZrHUL.js";
import { a as parseCliContainerArgs, i as maybeRunCliInContainer, n as applyCliProfileEnv, r as parseCliProfileArgs, t as normalizeWindowsArgv } from "./windows-argv-DDAf3waz.js";
import { n as defaultRuntime } from "./runtime-kS8e4c6-.js";
import { a as enableConsoleCapture } from "./subsystem-C9VSlEcV.js";
import { r as normalizeEnv, t as isTruthyEnvValue } from "./env-CK3eZNIX.js";
import { _ as resolveStateDir } from "./paths-GWMNxnBn.js";
import { t as assertSupportedRuntime } from "./runtime-guard-78ESz4Qy.js";
import "./logging-CAfgK7sj.js";
import { o as hasMemoryRuntime } from "./memory-state-DDJh6L4-.js";
...
const routeHealth = {
	...
	run: async (argv) => {
		...
		const { healthCommand } = await import("./health-D9lW8NQC.js");
		await healthCommand({
			json,
			timeoutMs,
			verbose
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		if (timeoutMs === null) return false;
		if (json) {
			const { statusJsonCommand } = await import("./status-json-CIASd7Of.js");
		...
		const { statusCommand } = await import("./status-CYovgFGh.js");
		await statusCommand({
			json,
			deep,
			all,
...
const routeGatewayStatus = {
	...
	run: async (argv) => {
		...
		const { runDaemonStatus } = await import("./status-ogZ_UO80.js");
...
const routeSessions = {
	...
	run: async (argv) => {
		...
		const { sessionsCommand } = await import("./sessions-DEZuACGJ.js");
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-B3PC9V8s.js");
...
const routes = [
	...
	routeAgentsList,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-BNzPbRj2.js");
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			...
			const pathArg = positionals[0];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-BNzPbRj2.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-Ov9eoGW4.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-Ov9eoGW4.js");
...
async function prepareRoutedCommand(params) {
	...
	if (!suppressDoctorStdout && process.stdout.isTTY) {
		const [{ emitCliBanner }, { VERSION }] = await Promise.all([import("./banner-BzPJu_TR.js"), import("./version-Dw0eJ-1F.js")]);
		emitCliBanner(VERSION, { argv: params.argv });
	}
	if (!skipConfigGuard) {
		const { ensureConfigReady } = await import("./config-guard-DIzau9MC.js");
	...
	if (typeof params.loadPlugins === "function" ? params.loadPlugins(params.argv) : params.loadPlugins) {
		const { ensurePluginRegistryLoaded } = await import("./plugin-registry-BNBxlqnj.js");
...
async function closeCliMemoryManagers() {
	...
	try {
		const { closeActiveMemorySearchManagers } = await import("./memory-runtime-DWwmC4FX.js");
...
async function runCli(argv = process$1.argv) {
	...
	if (shouldLoadCliDotEnv()) {
		const { loadCliDotEnv } = await import("./dotenv-DhJmHrKG.js");
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			const { outputRootHelp } = await import("./root-help-C56v-rtM.js");
			await outputRootHelp();
			return;
		}
		if (await tryRouteCli(normalizedArgv)) return;
		enableConsoleCapture();
		const { buildProgram } = await import("./program-UbyQCsgx.js");
		const program = buildProgram();
		const { installUnhandledRejectionHandler } = await import("./unhandled-rejections-CA_e1R-O.js");
		...
		if (primary) {
			const { getProgramContext } = await import("./program-context-B9LgGHBq.js");
			const ctx = getProgramContext(program);
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-DQirHZq4.js");
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-FsUIZ35V.js");
		...
		if (!shouldSkipPluginCommandRegistration({
			...
			const { loadValidatedConfigForPluginRegistration } = await import("./register.subclis-FsUIZ35V.js");
