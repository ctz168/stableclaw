import { i as restoreTerminalState, n as defaultRuntime, r as writeRuntimeJson } from "./runtime-kS8e4c6-.js";
...
import { s as isValidEnvSecretRefId } from "./types.secrets-BpcqHPAv.js";
...
import "./daemon-runtime-DPejcJWG.js";
import { c as normalizeGatewayTokenInput, d as randomToken, f as resolveControlUiLinks, g as waitForGatewayReachable, n as applyWizardMetadata, o as handleReset, r as ensureWorkspaceAndSessions, t as DEFAULT_WORKSPACE } from "./onboard-helpers-BTKcMkph.js";
...
import { a as resolveManifestProviderOnboardAuthFlags } from "./provider-auth-choices-BCpcD6rj.js";
import { i as resolveDeprecatedAuthChoiceReplacement, n as isDeprecatedAuthChoice, r as normalizeLegacyOnboardAuthChoice, t as formatDeprecatedNonInteractiveAuthChoiceError } from "./auth-choice-legacy-DCLTe62o.js";
import { t as runSetupWizard } from "./setup-CvYtDmX6.js";
import { r as applyLocalSetupWorkspaceConfig } from "./onboard-config-CR_rJ-iK.js";
...
async function collectGatewayHealthFailureDiagnostics() {
	const diagnostics = {};
	try {
		const { resolveGatewayService } = await import("./service-BCUDr0mF.js");
	...
	try {
		const { readLastGatewayErrorLine } = await import("./diagnostics-GjF7wpY9.js");
...
async function runNonInteractiveLocalSetup(params) {
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-CFA1M7LM.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-DEyKod0t.js");
	...
	if (!opts.json) runtime.log(`Tip: run \`${formatCliCommand("openclaw configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.stableclaw.ai/tools/web`);
}
//#endregion
...
async function runNonInteractiveRemoteSetup(params) {
	...
	if (opts.json) writeRuntimeJson(runtime, payload);
		...
		runtime.log(`Tip: run \`${formatCliCommand("openclaw configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.stableclaw.ai/tools/web`);
	}
}
//#endregion
...
async function setupWizardCommand(opts, runtime = defaultRuntime) {
	...
	if (normalizedOpts.nonInteractive && normalizedOpts.acceptRisk !== true) {
		runtime.error([
			...
			"Read: https://docs.stableclaw.ai/security",
	...
	if (process.platform === "win32") runtime.log([
		...
		"Guide: https://docs.stableclaw.ai/windows"
	].join("\n"));
