import { o as resolveRequiredHomeDir, t as expandHomePrefix } from "./home-dir-n9onX5PG.js";
import { E as isPlainObject$1, u as isRecord$1, v as resolveUserPath } from "./utils-CN_F_3Qg.js";
import { t as DEFAULT_CONTEXT_TOKENS } from "./defaults-DAt--D9n.js";
...
import { f as parseModelRef } from "./model-selection-w1RJdjfl.js";
import { _ as resolveStateDir, n as DEFAULT_GATEWAY_PORT } from "./paths-GWMNxnBn.js";
import { c as normalizeAgentId } from "./session-key-Do__tq1E.js";
...
import { a as mergeMissing, i as mapLegacyAudioTranscription, n as ensureRecord$1, r as getRecord, t as defineLegacyConfigMigration } from "./legacy.shared-oZM32end.js";
import { C as resolveSlackNativeStreaming, S as resolveDiscordPreviewStreamMode, T as resolveTelegramPreviewStreamMode, a as IrcConfigSchema, b as formatSlackStreamingBooleanMigrationMessage, c as SlackConfigSchema, i as IMessageConfigSchema, l as TelegramConfigSchema, n as DiscordConfigSchema, o as MSTeamsConfigSchema, r as GoogleChatConfigSchema, s as SignalConfigSchema, t as BlueBubblesConfigSchema, v as ChannelHeartbeatVisibilitySchema, w as resolveSlackStreamingMode, y as formatSlackStreamModeMigrationMessage } from "./zod-schema.providers-core-Bmbs-R4l.js";
import { g as CHANNEL_IDS, r as normalizeChatChannelId } from "./chat-meta-wVbKo1sw.js";
import { a as hasConfiguredSecretInput, i as coerceSecretRef } from "./types.secrets-BpcqHPAv.js";
import { C as SecretInputSchema, E as SecretsConfigSchema, L as createAllowDenyChannelRulesSchema, P as TypingModeSchema, _ as NativeCommandsSettingSchema, c as GroupChatSchema, d as HumanDelaySchema, g as ModelsConfigSchema, i as ContextVisibilityModeSchema, j as TtsConfigSchema, k as TranscribeAudioSchema, l as GroupPolicySchema, n as BlockStreamingCoalesceSchema, p as InboundDebounceSchema, r as CliBackendSchema, t as BlockStreamingChunkSchema, u as HexColorSchema, y as QueueSchema } from "./zod-schema.core-C1Ewlj9j.js";
import { t as sensitive } from "./zod-schema.sensitive-DEzKzMxI.js";
import { o as BUNDLED_WEB_SEARCH_PLUGIN_IDS$1 } from "./bundled-capability-metadata-BpPgIy_5.js";
import { r as hasKind } from "./slots-Dq-2eIRb.js";
import { a as normalizePluginsConfig, l as resolveMemorySlotDecision, s as resolveEffectivePluginActivationState } from "./config-state-DfIk9ME6.js";
import { t as withBundledPluginAllowlistCompat } from "./bundled-compat-BLlVmkDi.js";
import { n as loadPluginManifestRegistry } from "./manifest-registry-Kx5Z2lJL.js";
import { n as appendAllowedValuesHint, r as summarizeAllowedValues, t as validateJsonSchemaValue } from "./schema-validator-DofqBvJi.js";
import { c as isWindowsAbsolutePath, i as isAvatarHttpUrl, n as hasAvatarUriScheme, o as isPathWithinRoot, r as isAvatarDataUrl } from "./avatar-policy-pho6U35i.js";
import { i as isCanonicalDottedDecimalIPv4, u as isLoopbackIpAddress } from "./ip-BZP18dDl.js";
import { t as splitShellArgs } from "./shell-argv-B9p91CMW.js";
import { t as parseDurationMs } from "./parse-duration-D9q1V0xx.js";
import { a as MemorySearchSchema, c as AgentModelSchema, i as HeartbeatSchema, n as AgentSandboxSchema, r as ElevatedAllowFromSchema, s as ToolsSchema, t as AgentEntrySchema } from "./zod-schema.agent-runtime-DFC7a3eV.js";
...
const AgentDefaultsSchema = z.object({
	...
	timeoutSeconds: z.number().int().min(0).max(600).optional().describe("Agent execution timeout in seconds. Default: 180 (3 minutes). Max: 600 (10 minutes). Set to 0 to disable timeout."),
	...
	subagents: z.object({
		...
		runTimeoutSeconds: z.number().int().min(0).max(600).optional().describe("Sub-agent execution timeout in seconds. Default: 180 (3 min). Max: 600 (10 min). Set to 0 to disable."),
...
const SECRETREF_POLICY_DOC_URL = "https://docs.stableclaw.ai/reference/secretref-credential-surface";
...
function mapZodIssueToConfigIssue(issue) {
	...
	const configIssue = {
		path,
		message: allowedValuesSummary ? appendAllowedValuesHint(message, allowedValuesSummary) : message,
	...
	if (code === "invalid_type") {
		...
		if (expected === "string" && received === "undefined") configIssue.suggestion = `Add a string value for "${path}" in your configuration file.`;
		else if (expected === "number" && received === "string") configIssue.suggestion = `Convert the string value to a number. Remove quotes around numeric values.`;
		else if (expected === "boolean" && (received === "string" || received === "number")) configIssue.suggestion = `Use true or false (without quotes) instead of ${received}.`;
		else if (expected === "array" && received === "object") configIssue.suggestion = `Use square brackets [] instead of curly braces {} for array values.`;
		else if (expected === "object" && received === "array") configIssue.suggestion = `Use curly braces {} instead of square brackets [] for object values.`;
	}
	if (code === "invalid_literal" && /required/i.test(message)) configIssue.suggestion = `Add the required field "${path}" to your configuration.`;
	if (code === "invalid_enum_value" || code === "invalid_value") {
		if (allowedValuesSummary && allowedValuesSummary.values.length > 0) configIssue.suggestion = `Use one of the allowed values: ${allowedValuesSummary.values.map((v) => typeof v === "string" ? `"${v}"` : String(v)).join(", ")}.`;
	}
	if (message.includes("Expected") && message.includes("but received")) {
		if (message.includes("JSON")) configIssue.suggestion = "Check for common JSON syntax errors: missing commas, unmatched brackets, or improper quotes.";
	}
	if (code === "unrecognized_keys" && record?.keys) {
		const keys = Array.isArray(record.keys) ? record.keys : [];
		if (keys.length > 0) configIssue.suggestion = `Remove or rename the unknown key(s): ${keys.map((k) => `"${k}"`).join(", ")}. Check for typos or consult the documentation.`;
	}
	return configIssue;
}
...
//#endregion
export { unwrapDispatchWrappersForResolution as $, parseExecArgvToken as A, resolveExecutableFromPathEnv as B, buildSafeShellCommand as C, splitCommandChainWithOperators as D, splitCommandChain as E, resolveExecutionTargetCandidatePath as F, extractShellWrapperInlineCommand as G, POSIX_SHELL_WRAPPERS as H, resolveExecutionTargetResolution as I, unwrapKnownShellMultiplexerInvocation as J, hasEnvManipulationBeforeShellWrapper as K, resolvePolicyAllowlistCandidatePath as L, resolveApprovalAuditCandidatePath as M, resolveCommandResolution as N, windowsEscapeArg as O, resolveCommandResolutionFromArgv as P, isDispatchWrapperExecutable as Q, resolvePolicyTargetCandidatePath as R, buildSafeBinsShellCommand as S, findDuplicateAgentDirs as St, resolvePlannedSegmentArgv as T, POWERSHELL_WRAPPERS as U, resolveExecWrapperTrustPlan as V, extractShellWrapperCommand as W, POWERSHELL_INLINE_COMMAND_FLAGS as X, POSIX_INLINE_COMMAND_FLAGS as Y, resolveInlineCommandMatch as Z, listRiskyConfiguredSafeBins as _, applyLegacyMigrations as _t, validateConfigObjectWithPlugins as a, resolveSafeBinProfiles as at, analyzeShellCommand as b, ensureControlUiAllowedOriginsForNonLoopbackBind as bt, parseByteSize as c, buildTalkConfigResponse as ct, materializeRuntimeConfig as d, resolveAgentMaxConcurrent as dt, unwrapKnownDispatchWrapperInvocation as et, getTrustedSafeBinDirs as f, resolveSubagentMaxConcurrent as ft, validateSafeBinArgv as g, listBundledWebSearchPluginIds as gt, normalizeTrustedSafeBinDirs as h, GENERATED_BUNDLED_CHANNEL_CONFIG_METADATA as ht, validateConfigObjectRawWithPlugins as i, normalizeSafeBinProfileFixtures as it, resolveAllowlistCandidatePath as j, matchAllowlist as k, asResolvedSourceConfig as l, normalizeTalkSection as lt, listWritableExplicitTrustedSafeBinDirs as m, collectPluginSchemaMetadata as mt, validateConfigObject as n, DEFAULT_SAFE_BINS as nt, OpenClawSchema as o, resolveNormalizedProviderModelMaxTokens as ot, isTrustedSafeBinPath as p, collectChannelSchemaMetadata as pt, isShellWrapperExecutable as q, validateConfigObjectRaw as r, SAFE_BIN_PROFILES as rt, parseNonNegativeByteSize as s, LEGACY_TALK_PROVIDER_ID as st, collectUnsupportedSecretRefPolicyIssues as t, normalizeExecutableToken as tt, asRuntimeConfig as u, resolveActiveTalkProviderConfig as ut, normalizeSafeBinName as v, findLegacyConfigIssues as vt, isWindowsPlatform as w, buildEnforcedShellCommand as x, DuplicateAgentDirError as xt, analyzeArgvCommand as y, migrateLegacyXSearchConfig as yt, resolvePolicyTargetResolution as z };
