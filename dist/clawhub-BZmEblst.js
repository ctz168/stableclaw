...
import { a as fetchClawHubPackageVersion, d as satisfiesPluginApiRange, i as fetchClawHubPackageDetail, l as resolveLatestVersionFromPackage, n as downloadClawHubPackageArchive, s as parseClawHubPluginSpec, t as ClawHubRequestError, u as satisfiesGatewayMinimum } from "./clawhub-Bi6Jol-I.js";
import { n as installPluginFromArchive } from "./install-DQYBvGSU.js";
...
function validateClawHubPluginPackage(params) {
	...
	if (compatibility?.pluginApiRange && !satisfiesPluginApiRange(runtimeVersion, compatibility.pluginApiRange)) return buildClawHubInstallFailure(`Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this StableClaw runtime exposes ${runtimeVersion}.`, CLAWHUB_INSTALL_ERROR_CODE.INCOMPATIBLE_PLUGIN_API);
	if (compatibility?.minGatewayVersion && !satisfiesGatewayMinimum(runtimeVersion, compatibility.minGatewayVersion)) return buildClawHubInstallFailure(`Plugin "${pkg.name}" requires StableClaw >=${compatibility.minGatewayVersion}, but this host is ${runtimeVersion}.`, CLAWHUB_INSTALL_ERROR_CODE.INCOMPATIBLE_GATEWAY);
	return null;
}
