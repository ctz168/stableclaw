...
import { m as shouldPreferNativeJiti } from "./chat-meta-wVbKo1sw.js";
...
const MATRIX_LEGACY_CRYPTO_INSPECTOR_UNAVAILABLE_MESSAGE = "Legacy Matrix encrypted state was detected, but the Matrix plugin helper is unavailable. Install or repair @stableclaw/matrix so OpenClaw can inspect the old rust crypto store before upgrading.";
...
async function loadMatrixLegacyCryptoInspector(params) {
	...
	if (resolution.status === "unsafe") throw new Error(`Matrix plugin helper path is unsafe: ${resolution.candidatePath}. Reinstall @stableclaw/matrix and try again.`);
	...
	const pending = (async () => {
		...
		if (!inspectLegacyMatrixCryptoStore) throw new Error(`Matrix plugin helper at ${helperPath} does not export inspectLegacyMatrixCryptoStore(). Reinstall @stableclaw/matrix and try again.`);
...
const MATRIX_MIGRATION_SNAPSHOT_DIRNAME = "stableclaw-migrations";
