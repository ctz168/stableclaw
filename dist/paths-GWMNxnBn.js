import { o as resolveRequiredHomeDir, r as resolveHomeRelativePath } from "./home-dir-n9onX5PG.js";
...
function resolveIsNixMode(env = process.env) {
	return env.STABLECLAW_NIX_MODE === "1" || env.OPENCLAW_NIX_MODE === "1";
}
const isNixMode = resolveIsNixMode();
const LEGACY_STATE_DIRNAMES = [".clawdbot", ".openclaw"];
const NEW_STATE_DIRNAME = ".stableclaw";
const CONFIG_FILENAME = "stableclaw.json";
const LEGACY_CONFIG_FILENAMES = ["clawdbot.json", "openclaw.json"];
function resolveDefaultHomeDir() {
	return resolveRequiredHomeDir(process.env, os.homedir);
}
/** Build a homedir thunk that respects STABLECLAW_HOME (or legacy OPENCLAW_HOME) for the given env. */
...
function resolveStateDir(env = process.env, homedir = envHomedir(env)) {
	...
	if (env.STABLECLAW_TEST_FAST === "1" || env.OPENCLAW_TEST_FAST === "1") return newDir;
...
function resolveCanonicalConfigPath(env = process.env, stateDir = resolveStateDir(env, envHomedir(env))) {
	const override = env.STABLECLAW_CONFIG_PATH?.trim() || env.OPENCLAW_CONFIG_PATH?.trim();
...
function resolveConfigPathCandidate(env = process.env, homedir = envHomedir(env)) {
	if (env.STABLECLAW_TEST_FAST === "1" || env.OPENCLAW_TEST_FAST === "1") return resolveCanonicalConfigPath(env, resolveStateDir(env, homedir));
...
function resolveConfigPath(env = process.env, stateDir = resolveStateDir(env, envHomedir(env)), homedir = envHomedir(env)) {
	...
	if (env.STABLECLAW_TEST_FAST === "1" || env.OPENCLAW_TEST_FAST === "1") return path.join(stateDir, CONFIG_FILENAME);
	const stateOverride = env.STABLECLAW_STATE_DIR?.trim() || env.OPENCLAW_STATE_DIR?.trim();
...
function resolveDefaultConfigCandidates(env = process.env, homedir = envHomedir(env)) {
	const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
	const explicit = env.STABLECLAW_CONFIG_PATH?.trim() || env.OPENCLAW_CONFIG_PATH?.trim();
	if (explicit) return [resolveUserPath(explicit, env, effectiveHomedir)];
	const candidates = [];
	const stateDirOverride = env.STABLECLAW_STATE_DIR?.trim() || env.OPENCLAW_STATE_DIR?.trim();
	if (stateDirOverride) {
...
function resolveGatewayLockDir(tmpdir = os.tmpdir) {
	...
	const suffix = uid != null ? `stableclaw-${uid}` : "stableclaw";
...
function resolveOAuthDir(env = process.env, stateDir = resolveStateDir(env, envHomedir(env))) {
	const override = env.STABLECLAW_OAUTH_DIR?.trim() || env.OPENCLAW_OAUTH_DIR?.trim();
	if (override) return resolveUserPath(override, env, envHomedir(env));
...
function resolveGatewayPort(cfg, env = process.env) {
	const envPort = parseGatewayPortEnvValue(env.STABLECLAW_GATEWAY_PORT?.trim() || env.OPENCLAW_GATEWAY_PORT?.trim());
