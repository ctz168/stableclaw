...
import "./runtime-B8hqEtu6.js";
import { n as loadPluginManifestRegistry, r as clearPluginDiscoveryCache } from "./manifest-registry-Kx5Z2lJL.js";
...
import { i as resolveBundledPluginSources, n as findBundledPluginSourceInMap } from "./bundled-sources-C2WkVzZw.js";
import { r as installPluginFromNpmSpec } from "./install-DQYBvGSU.js";
import { n as recordPluginInstall, t as buildNpmResolutionInstallFields } from "./installs-0wxqHZSR.js";
import { n as getChannelPluginCatalogEntry } from "./catalog-CwR2AkIk.js";
import { n as resolveBundledInstallPlanForCatalogEntry } from "./plugin-install-plan-CpN5c1mm.js";
...
async function ensureChannelSetupPluginInstalled(params) {
	...
	if (result.ok) {
		...
		try {
			const { loadNewPlugin } = await import("./plugin-hot-reload-OV8EW87o.js");
			const reloadResult = await loadNewPlugin({
				pluginId: result.pluginId,
				installPath: result.targetDir,
				config: next
			});
			if (reloadResult.ok) runtime.log?.(`Plugin ${result.pluginId} hot-reloaded successfully`);
			else runtime.log?.(`Plugin ${result.pluginId} hot-reload failed: ${reloadResult.error}`);
		} catch (error) {
