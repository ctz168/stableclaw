...
function buildServiceEnvironment(params) {
	...
	const profile = env.STABLECLAW_PROFILE || env.OPENCLAW_PROFILE;
	...
	return {
		...
		STABLECLAW_PROFILE: profile,
		STABLECLAW_GATEWAY_PORT: String(port),
		STABLECLAW_LAUNCHD_LABEL: resolvedLaunchdLabel,
		STABLECLAW_SYSTEMD_UNIT: systemdUnit,
		STABLECLAW_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
		STABLECLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
		STABLECLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
		STABLECLAW_SERVICE_VERSION: VERSION
	};
}
function buildNodeServiceEnvironment(params) {
	...
	const gatewayToken = (env.STABLECLAW_GATEWAY_TOKEN || env.OPENCLAW_GATEWAY_TOKEN)?.trim() || void 0;
	return {
		...buildCommonServiceEnvironment(env, sharedEnv),
		STABLECLAW_GATEWAY_TOKEN: gatewayToken,
		STABLECLAW_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
		STABLECLAW_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
		STABLECLAW_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
		STABLECLAW_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
		STABLECLAW_LOG_PREFIX: "node",
		STABLECLAW_SERVICE_MARKER: NODE_SERVICE_MARKER,
		STABLECLAW_SERVICE_KIND: NODE_SERVICE_KIND,
		STABLECLAW_SERVICE_VERSION: VERSION
	};
}
function buildCommonServiceEnvironment(env, sharedEnv) {
	const serviceEnv = {
		...
		STABLECLAW_STATE_DIR: sharedEnv.stateDir,
		STABLECLAW_CONFIG_PATH: sharedEnv.configPath
	};
...
function resolveSharedServiceEnvironmentFields(env, platform, extraPathDirs, execPath) {
	const stateDir = env.STABLECLAW_STATE_DIR || env.OPENCLAW_STATE_DIR;
	const configPath = env.STABLECLAW_CONFIG_PATH || env.OPENCLAW_CONFIG_PATH;
