...
import { n as defaultRuntime, r as writeRuntimeJson } from "./runtime-kS8e4c6-.js";
import { d as setConsoleTimestampPrefix, t as createSubsystemLogger, u as setConsoleSubsystemFilter } from "./subsystem-C9VSlEcV.js";
import { v as resolveUserPath, x as shortenHomePath } from "./utils-CN_F_3Qg.js";
...
import { _ as resolveStateDir, t as CONFIG_PATH, u as resolveGatewayPort } from "./paths-GWMNxnBn.js";
import "./globals-CDtV0ht8.js";
import { _ as resolveWorkspaceTemplateDir, g as resolveDefaultAgentWorkspaceDir } from "./workspace-D5P9nuJd.js";
import { c as loadConfig, d as readBestEffortConfig, f as readConfigFileSnapshot, i as createConfigIO, x as writeConfigFile } from "./io-8kNEV_ou.js";
import { a as hasConfiguredSecretInput } from "./types.secrets-BpcqHPAv.js";
...
import { l as readGatewayPasswordEnv, u as readGatewayTokenEnv } from "./credentials-BGPs1gIX.js";
...
import { t as inspectBestEffortPrimaryTailnetIPv4 } from "./network-discovery-display-h8MqTMCI.js";
import { o as handleReset } from "./onboard-helpers-BTKcMkph.js";
import { t as cleanStaleGatewayProcessesSync } from "./restart-stale-pids-dsB3eE6_.js";
import { m as scheduleDetachedLaunchdRestartHandoff } from "./launchd-cp98MUhz.js";
import { a as inspectPortUsage, s as formatPortDiagnostics } from "./ports-ZA1n2It7.js";
import { t as parsePort } from "./parse-port-BFmNhrtE.js";
import { n as addGatewayServiceCommands } from "./daemon-cli-BLCPZV1F.js";
import { i as probeGateway } from "./probe-RbbTQOHx.js";
import { t as resolveConfiguredSecretInputString } from "./resolve-configured-secret-input-string-BasQGOd9.js";
import { n as withProgress } from "./progress-DQvXYKne.js";
import { l as waitForActiveTasks, o as markGatewayDraining, r as getActiveTaskCount, s as resetAllLanes } from "./command-queue-B42AiLXM.js";
import { _ as markGatewaySigusr1RestartHandled, d as waitForActiveEmbeddedRuns, g as isGatewaySigusr1RestartExternallyAllowed, i as getActiveEmbeddedRunCount, p as consumeGatewaySigusr1RestartAuthorization, t as abortEmbeddedPiRun, v as scheduleGatewaySigusr1Restart, x as triggerStableClawRestart } from "./runs-CDodmdPt.js";
import { n as parseTimeoutMsWithFallback } from "./parse-timeout-bktz0495.js";
import { n as formatTokenCount, r as formatUsd } from "./usage-format-C52ZQlyn.js";
import { n as runCommandWithRuntime } from "./cli-utils-qgKJn7xy.js";
import { t as formatHelpExamples } from "./help-format-5ems77mC.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-DB5pVzxA.js";
import { t as readSecretFromFile } from "./secret-file-lbOD3X5Z.js";
import { n as resolveWideAreaDiscoveryDomain } from "./widearea-dns-DfghnpNq.js";
import { i as toOptionString, n as extractGatewayMiskeys, r as maybeExplainGatewayServiceStop, t as describeUnknownError } from "./shared-0SE-ELxR.js";
import { i as discoverGatewayBeacons, n as buildGatewayDiscoveryTarget, r as serializeGatewayDiscoveryBeacon } from "./gateway-discovery-targets-Iv8qDi-e.js";
import { t as pickGatewaySelfPresence } from "./gateway-presence-BOfOY7Mo.js";
import { i as styleHealthChannelLine, t as formatHealthChannelLines } from "./health-CYC7-jZ6.js";
import { c as wasLastConfigInvalid, n as getConfigStatus } from "./config-status-BbdRP0Sw.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-Be-VwfrK.js";
import { n as forceFreePortAndWait, r as waitForPortBindable } from "./ports-BMS5viOy.js";
...
function loadSshConfigModule() {
	sshConfigModulePromise ??= import("./ssh-config-BroZL4Mv.js");
	return sshConfigModulePromise;
}
function loadSshTunnelModule() {
	sshTunnelModulePromise ??= import("./ssh-tunnel-D5YdH2SQ.js");
...
async function loadDevTemplate(name, fallback) {
	try {
		...
		const raw = await fsSync.promises.readFile(path.join(templateDir, name), "utf-8");
...
async function writeFileIfMissing(filePath, content) {
	try {
		await fsSync.promises.writeFile(filePath, content, {
			encoding: "utf-8",
			flag: "wx"
		});
	} catch (err) {
...
async function ensureDevWorkspace(dir) {
	...
	await fsSync.promises.mkdir(resolvedDir, { recursive: true });
...
function restartGatewayProcessWithFreshPid() {
	...
	if (supervisor) {
		...
		if (supervisor === "schtasks") {
			const restart = triggerStableClawRestart();
...
async function runGatewayLoop(params) {
	gatewayLog$1.info("acquiring gateway lock...");
	let lock = await acquireGatewayLock({ port: params.lockPort });
	gatewayLog$1.info("gateway lock acquired");
	let server = null;
	let shuttingDown = false;
	let restartResolver = null;
	let healthChecker = null;
	try {
		const { getPluginHealthChecker } = await import("./plugin-health-checker-LlGmFLvG.js");
		healthChecker = getPluginHealthChecker();
		healthChecker.start();
		gatewayLog$1.info("plugin health checker started");
	} catch (error) {
		gatewayLog$1.warn(`Failed to start plugin health checker: ${String(error)}`);
	}
...
*/
async function promptKillOrExit(existingPid) {
	...
	return new Promise((resolve) => {
		...
		rl.question("", (answer) => {
			...
			if (trimmed === "k" || trimmed === "kill" || trimmed === "y" || trimmed === "yes" || trimmed === "") resolve("kill");
			else resolve("exit");
		});
		rl.on("close", () => {
			resolve("exit");
...
async function runGatewayCommand$1(opts) {
	...
	const { startGatewayServer } = await withProgress({
		...
		indeterminate: true
	}, async () => import("./server-COHYqlDP.js"));
	setConsoleTimestampPrefix(true);
	if (devMode) await ensureDevGatewayConfig({ reset: Boolean(opts.reset) });
	gatewayLog.info("loading configuration…");
	const { setupGlobalPluginErrorHandlers } = await import("./plugin-error-boundary-CzocNII7.js");
	...
	if (wasLastConfigInvalid()) {
		...
		gatewayLog.warn(`Previous session had an invalid configuration that was rolled back automatically.`);
		gatewayLog.warn(`  Last error: ${errorDetail}`);
		if (invalidPath) gatewayLog.warn(`  Invalid config preserved at: ${invalidPath}`);
		if (backupPath) gatewayLog.warn(`  Rolled back from backup: ${backupPath}`);
		gatewayLog.warn(`  Please review the invalid config and fix the root cause before making further changes.`);
		const { clearConfigStatus } = await import("./config-status-BLo5Zc5J.js");
		clearConfigStatus();
	}
	...
	const configExists = snapshot?.exists ?? fsSync.existsSync(CONFIG_PATH);
	const configAuditPath = path.join(resolveStateDir(process.env), "logs", "config-audit.jsonl");
	const mode = (snapshot?.valid ? snapshot.config : cfg).gateway?.mode ?? "local";
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			if (process.stdin.isTTY && !opts.force && existingPid) {
				defaultRuntime.error(`Gateway failed to start: ${errMessage}`);
				defaultRuntime.error("");
				if (await promptKillOrExit(existingPid) === "kill") {
