...
async function ensureMatrixCryptoRuntime(...args) {
	const { ensureMatrixCryptoRuntime: ensureRuntime } = await import("../../deps-CfdT-aQv.js");
	await ensureRuntime(...args);
}
async function handleVerifyRecoveryKey({ params, respond }) {
	try {
		const { verifyMatrixRecoveryKey } = await import("../../verification-BG2ZRKjD.js");
...
async function handleVerificationBootstrap({ params, respond }) {
	try {
		const { bootstrapMatrixVerification } = await import("../../verification-BG2ZRKjD.js");
...
async function handleVerificationStatus({ params, respond }) {
	try {
		const { getMatrixVerificationStatus } = await import("../../verification-BG2ZRKjD.js");
