# StableClaw ACP Bridge

This document describes how the StableClaw ACP (Agent Client Protocol) bridge works,
...
StableClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing StableClaw Gateway
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the StableClaw gateway or agent instead.                                                                                                                                     |
...
drive a StableClaw Gateway session.
...
In Zed, open the Agent panel and select “StableClaw ACP” to start a thread.
